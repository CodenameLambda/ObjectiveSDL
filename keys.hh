...
namespace SDL {
    ...
    Key key_from_name(const std::string&);
    Key key_from_name(std::string&&);
    Key key_from_scankey(const ScanKey);
    std::string get_key_name(const Key);
    Window& get_keyboard_focus();
    SDL_Keymod get_modifiers();
    void set_modifiers(SDL_Keymod);
    ScanKey get_scankey_from_key(const Key);
    ScanKey get_scankey_from_name(const std::string&);
    ScanKey get_scankey_from_name(std::string&&);
    std::string get_scankey_name(const ScanKey);
    bool has_screen_keyboard_support();
    bool screen_keyboard_shown(Window&);
    bool text_input_active();
    void start_text_input();
    void stop_text_input();
    void set_text_input_rectangle(const RectangleI);

    boost::dynamic_bitset<> get_keyboard_state();  // the keys are of the type ScanKey
}
